...
/**
 * Creates arrays of objects. Returns a primitive type array such as Int Array if
 * the objects are Ints, etc. Returns an ArrayBuffer[Any] if it needs to
...
class ArrayConstructor extends IObjectConstructor:
          ...
          case values: ArrayBuffer[Any] =>
            typecode.charAt(0) match
              ...
              case 'd' =>
                ...
                result.asInstanceOf[AnyRef]
              case _ =>
                throw PickleException(s"invalid array typecode: $typecode")
      case _ =>
        throw PickleException(s"invalid pickle data for array; expected 2 args, got ${args.length}")


  def construct(typecode: Char, machinecode: Int, data: Array[Byte]): AnyRef =
    if machinecode < 0 then
      throw PickleException("unknown machine type format")

    typecode match
      case 'c' | 'u' =>
        if machinecode != 18 && machinecode != 19 && machinecode != 20 && machinecode != 21 then
          throw PickleException("for c/u type must be 18/19/20/21")
      ...
      case 'b' =>
        if machinecode != 1 then
          throw PickleException("for b type must be 1")
        data.asInstanceOf[AnyRef]
      case 'B' =>
        if machinecode != 0 then
          throw PickleException("for B type must be 0")
      ...
      case 'l' =>
        if machinecode != 8 && machinecode != 9 && machinecode != 12 && machinecode != 13 then
          throw PickleException("for l type must be 8/9/12/13")
      ...
      case 'L' =>
        if machinecode != 6 && machinecode != 7 && machinecode != 10 && machinecode != 11 then
          throw PickleException("for L type must be 6/7/10/11")
  ...
  protected def constructShortArraySigned(machinecode: Int, data: Array[Byte]): Array[Short] =
    ...
    for (i <- 0 until data.length / 2) {
      val b1 = data(0 + i * 2)
      val b2 = data(1 + i * 2)
      if machinecode == 4 then
        result(i) = ((b2 << 8) | (b1 & 0xff)).toShort
      else
  ...
  protected def constructCharArrayUTF32(machinecode: Int, data: Array[Byte]): Array[Char] =
    ...
    for (index <- 0 until data.length / 4) {
      if machinecode == 20 then
        ...
        if cc.length > 1 then
          throw PickleException(s"cannot process UTF-32 character codepoint $codepoint")
        result(index) = cc(0)
      else
