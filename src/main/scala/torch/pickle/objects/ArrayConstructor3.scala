...
//    if (args.length == 4) {
//      val constructor = args(0).asInstanceOf[ArrayConstructor3]
...
//    }
//    @SuppressWarnings(Array("unchecked")) val values = args(1).asInstanceOf[Seq[AnyRef]]
...
//        constructIntArrayFromUShort(machinecode, data)// unsigned integer 2 -> int[]
//      case 'i' =>
//        if machinecode != 8 && machinecode != 9 then
//          throw PickleException("for i type must be 8/9")
//        if data.length % 4 != 0 then
//          throw PickleException("data size alignment error")
//        constructIntArrayFromInt32(machinecode, data)// signed integer 2 -> int[]
