 ...
 * 
 */
class ExceptionConstructor(private val types: Class[?], module: String, name: String) extends IObjectConstructor {
 
  final private var pythonExceptionType: String =  if (module != null)  module + "." + name
  else  name

  override def construct(param: Array[AnyRef]): AnyRef = try {
    var args = param
    if (pythonExceptionType != null) {
      // put the python exception type somewhere in the message
      if (args == null || args.length == 0) args = Array[AnyRef]("[" + pythonExceptionType + "]")
      else {
