...
class DateTimeConstructor(private val pythontype: Int)
  ...
  private def createTimedelta(args: Array[AnyRef]) = {
    ...
    if (args.length != 3) throw new PickleException(
      "invalid pickle data for timedelta; expected 3 args, got " + args.length,
    )
  ...
  private def createDateTime(args: Array[AnyRef]): Calendar = {
    ...
    if (args.length != 1 && args.length != 2) throw new PickleException(
      "invalid pickle data for datetime; expected 1, 2, 7 or 8 args, got " +
        args.length,
    )
    var yhi = 0
    var ylo = 0
    ...
    if (args(0).isInstanceOf[String]) {
      ...
      if (params.length != 10) throw new PickleException(
        "invalid pickle data for datetime; expected arg of length 10, got length " +
          params.length,
      )
      yhi = params.charAt(0)
      ylo = params.charAt(1)
    ...
    val cal =
      new GregorianCalendar(yhi * 256 + ylo, month, day, hour, minute, second)
    cal.set(Calendar.MILLISECOND, microsec / 1000)
    if (args.length == 2)
      // Timezone passed as the second constructor arg in pickle protocal 0
      if (args(1).isInstanceOf[TimeZone]) cal
        .setTimeZone(args(1).asInstanceOf[TimeZone])
      else if (args(1).isInstanceOf[Tzinfo]) cal
        .setTimeZone(args(1).asInstanceOf[Tzinfo].getTimeZone)
      else throw new PickleException(
        "invalid pickle data for datetime; expected arg 2 to be a Tzinfo or TimeZone",
      )
    cal
  }
  ...
  private def createTime(args: Array[AnyRef]): Time = {
    ...
    if (args.length != 1) throw new PickleException(
      "invalid pickle data for time; expected 1 or 4 args, got " + args.length,
    )
    var hour = 0
    var minute = 0
    var second = 0
    ...
    if (args(0).isInstanceOf[String]) {
      val params = args(0).asInstanceOf[String]
      if (params.length != 6) throw new PickleException(
        "invalid pickle data for time; expected arg of length 6, got length " +
          params.length,
      )
      ...
      val params = args(0).asInstanceOf[Array[Byte]]
      if (params.length != 6) throw new PickleException(
        "invalid pickle data for datetime; expected arg of length 6, got length " +
          params.length,
      )
      hour = params(0) & 0xff
  ...
  private def createDate(args: Array[AnyRef]): Calendar = {
    ...
    if (args.length != 1) throw new PickleException(
      "invalid pickle data for date; expected 1 arg, got " + args.length,
    )
    var yhi = 0
    var ylo = 0
    var month = 0
    ...
    if (args(0).isInstanceOf[String]) {
      val params = args(0).asInstanceOf[String]
      if (params.length != 4) throw new PickleException(
        "invalid pickle data for date; expected arg of length 4, got length " +
          params.length,
      )
      yhi = params.charAt(0)
      ylo = params.charAt(1)
