...
/** Helper class to mimic the datetime.timedelta Python object (holds a
  * days/seconds/microsec time difference).
  */
@SerialVersionUID(7655189815303876847L)
class TimeDelta(val days: Int, val seconds: Int, val microseconds: Int)
    extends Serializable {
  final var total_seconds = days * 86400 + seconds + microseconds / 1000000.0
  ...
  override def toString: String = {
    ...
    String.format(
      "Timedelta: %d days, %d seconds, %d microseconds (total: %s seconds)",
      days,
      seconds,
      microseconds,
      floatsecs,
  ...
  override def equals(obj: Any): Boolean = {
    ...
    microseconds == other.microseconds && total_seconds == other.total_seconds
  }
}
