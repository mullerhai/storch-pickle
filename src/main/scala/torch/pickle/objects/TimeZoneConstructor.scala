...
class TimeZoneConstructor(private val pythontype: Int)
  ...
  def reconstruct(baseConstructor: AnyRef, state: AnyRef): AnyRef = {
    if (!state.isInstanceOf[Tzinfo]) throw new PickleException(
      "invalid pickle data for tzinfo reconstruction; expected emtpy tzinfo state class",
    )
    if (!baseConstructor.isInstanceOf[TimeZoneConstructor])
      throw new PickleException(
        "invalid pickle data for tzinfo reconstruction; expected a TimeZoneConstructor from a known tzinfo subclass",
      )
    ...
    if (this.pythontype == TimeZoneConstructor.DATEUTIL_TZUTC) TimeZone
      .getTimeZone("UTC")
    else throw new PickleException(
      "unsupported pickle data for tzinfo reconstruction; support for tzinfo subclasses other than tztuc has not been implemented",
    )
  }
  ...
  private def createInfoFromDateutilTzfile(args: Array[AnyRef]) = {
    if (args.length != 1) throw new PickleException(
      "invalid pickle data for dateutil tzfile timezone; expected 1 args, got " +
        args.length,
    )
    ...
    if (index != -1) identifier = identifier.substring(index + 8 + 1)
      ...
      "couldn't parse timezone identifier from zoneinfo path" + identifier,
    )
  ...
  private def createInfoFromDateutilGettz(args: Array[AnyRef]) = {
    if (args.length != 1) throw new PickleException(
      "invalid pickle data for dateutil gettz call; expected 1 args, got " +
        args.length,
    )
  ...
  private def createZoneFromPytz(args: Array[AnyRef]) = {
    if (args.length != 4 && args.length != 1) throw new PickleException(
      "invalid pickle data for pytz timezone; expected 1 or 4 args, got " +
        args.length,
    )
    ...
    if (!args(0).isInstanceOf[String]) throw new PickleException(
      "invalid pickle data for pytz timezone; expected string argument as first tuple member",
    )
