...
//    }
    ...
    println(s"readline  sb ${sb.toString}")
    sb.toString
  }
  ...
  /** read a number of signed bytes into the specified location in an existing
    * byte array
    */
  @throws[IOException]
  def readbytes_into(
  ...
  /** Convert a couple of bytes into the corresponding integer number. Can deal
    * with 2-bytes unsigned int and 4-bytes signed int.
    */
  def bytes_to_integer(bytes: Array[Byte]): Int =
    bytes_to_integer(bytes, 0, bytes.length)
  ...
  def bytes_to_integer(bytes: Array[Byte], offset: Int, size: Int): Int =
    ...
    size match {
      ...
      case _ => throw PickleException(
          s"invalid amount of bytes to convert to int: $size",
        )
    }
//  def bytes_to_integer(bytes: Array[Byte], offset: Int, size: Int): Int =
//    // this operates on little-endian bytes
...
//  }
  ...
  /** Convert a big endian 8-byte to a double.
    */
  def bytes_to_double(bytes: Array[Byte], offset: Int): Double =
    try {
      var result: Long = bytes(0 + offset) & 0xff // must set to Long ,or it will be int then make error code！！！！
      println(s"bytes to double before result $result byte ${bytes.toSeq
          .mkString(",")}")
      result <<= 8
...
//    result = 4614680912179589284l
      val doubleResult = java.lang.Double.longBitsToDouble(result)
      println(s"bytes_to double result $result ,double result $doubleResult")
      doubleResult
    } catch {
  ...
  /** Convert a big endian 4-byte to a float.
    */
  def bytes_to_float(bytes: Array[Byte], offset: Int): Float =
    try {
  ...
  /** read an arbitrary 'long' number. Returns an int/long/BigInteger as
    * appropriate to hold the number.
    */
  def decode_long(data: Array[Byte]): Number = {
    ...
    for (i <- 0 until data.length) data2(data.length - i - 1) = data(i)
  ...
  /** Construct a String from the given bytes where these are directly converted
    * to the corresponding chars, without using a given character encoding
    */
  def rawStringFromBytes(data: Array[Byte]): String = {
    val str = new StringBuilder(data.length)
    for (b <- data) str.append((b & 0xff).toChar)
    str.toString
  }

  /** Convert a string to a byte array, no encoding is used. String must only
    * contain characters less than 256.
    */
  @throws[IOException]
  def str2bytes(str: String): Array[Byte] = {
    ...
    for (i <- 0 until str.length) {
      val c = str.charAt(i)
      if (c > 255) throw new UnsupportedEncodingException(
    ...
    while (i < str.length) {
      ...
      if (c == '\\') {
        ...
        c2 match {
          ...
          case 'x' =>
            // hex escaped "\x??"
            val h1 = str.charAt { i += 1; i }
            val h2 = str.charAt { i += 1; i }
            c2 = Integer.parseInt("" + h1 + h2, 16).toChar
            sb.append(c2)
          case 'n' => sb.append('\n')
          case 'r' => sb.append('\r')
          case 't' => sb.append('\t')
          case '\'' => sb.append('\'') // sometimes occurs in protocol level 0 strings
          case _ =>
            if (str.length > 80) str = str.substring(0, 80)
            throw new PickleException(
              "invalid escape sequence char '" + c2 + "' in string \"" + str +
                " [...]\" (possibly truncated)",
            )
        }
      ...
      if (c == '\\') {
        ...
        c2 match {
          ...
          case 'u' =>
            ...
            val h1 = str.charAt { i += 1; i }
            val h2 = str.charAt { i += 1; i }
            val h3 = str.charAt { i += 1; i }
            val h4 = str.charAt { i += 1; i }
          ...
          case 'U' =>
            ...
            val h3 = str.charAt { i += 1; i }
            val h4 = str.charAt { i += 1; i }
            val h5 = str.charAt { i += 1; i }
            val h6 = str.charAt { i += 1; i }
            val h7 = str.charAt { i += 1; i }
            val h8 = str.charAt { i += 1; i }
