...
//  }
  ...
  def dump(o: AnyRef, stream: OutputStream): Unit = {
    out = stream
    recurse = 0
    if (useMemo) memo = new mutable.HashMap[Int, Pickler.Memo]
  ...
  private def dispatch(t: Class[?], o: Any): Boolean = {
    ...
    if (o.isInstanceOf[Integer] || t == Integer.TYPE) {
    ...
    if (o.isInstanceOf[Time]) {
      val sqltime = o.asInstanceOf[java.sql.Time]
...
//      return true
//    } //todo need implement
    if (o.isInstanceOf[Set[?]]) {
      put_set(o.asInstanceOf[Set[?]])
      return true
    }
    ...
    if (o.isInstanceOf[Seq[?]]) {
      put_collection(o.asInstanceOf[Seq[?]])
      return true
    }
    // javabean
...
//    }
  ...
  @throws[IOException]
  private[pickle] def put_map(o: Map[?, ?]): Unit = {
  ...
  @throws[IOException]
  private[pickle] def put_set(o: Set[?]): Unit = {
